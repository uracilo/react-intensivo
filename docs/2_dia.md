...
| --- | --- | --- |
| `saving` | Indica que el POST está en curso | Deshabilita el botón Guardar del diálogo |
| `addUser` devuelve `boolean` | `true` si OK, `false` si error | La página cierra el diálogo solo si tuvo éxito |
| `setUsers((prev) => [...prev, ...])` | Agrega al array local | Ves el usuario nuevo sin recargar |
...
| | |
| --- | --- |
| **Archivo** | `src/components/UserFormDialog.tsx` |
| **Origen** | Vacío (Paso 0.2) |
| **Acción** | Pegá el código completo |
| **Qué ves** | 👁️ Sin cambio hasta el Paso 2.3 |
...
| Bloque | ¿Qué hace? | ¿Por qué importa? |
| --- | --- | --- |
| `initial?` | Valores precargados | Reutilizarás el mismo diálogo para editar en Fase 3 |
| `useEffect` al abrir | Resetea o precarga el form | Evita ver datos viejos al abrir de nuevo |
| `valid` | Comprueba campos mínimos | No envía POST con formulario vacío |
| `event.preventDefault()` | Evita recarga de página | Submit clásico de HTML dentro del diálogo |
...
| **Origen** | Versión de la Fase 1 |
| **Acción** | Reemplazá **todo** el contenido |
| **Qué ves** | 👁️ Botón arriba a la derecha + diálogo al hacer clic |
...
| Bloque | ¿Qué hace? | ¿Por qué importa? |
| --- | --- | --- |
| `useState(false)` para `open` | Controla si el diálogo está visible | Patrón open/close sin librerías extra |
| `handleCreate` | Llama `addUser` y cierra si OK | La lista se actualiza sola vía el hook |
| `startIcon={<AddIcon />}` | Icono + en el botón | Acción crear reconocible visualmente |
| `<UserFormDialog />` al final | Hermano del contenido principal | MUI Dialog se renderiza en portal — no afecta el layout |
...
1. Ves el botón **Nuevo usuario** arriba a la derecha.
2. Clic → se abre el diálogo con tres campos.
3. Completás Nombre, Usuario, Email → **Guardar**.
4. El diálogo se cierra y el usuario aparece al final de la lista.
5. Si dejás campos vacíos, **Guardar** permanece deshabilitado.
